...
def display_supplier_info(df, fournisseur):
 ...
 def display_designation_info(df, designation):
    ...
    if not df_filtered.empty:
        # Ajouter la colonne taille normalisée
        df_filtered['taille_normalisee'] = df_filtered['taille'].apply(normalize_size)

        # Grouper par taille et sommer les quantités disponibles
        ...
        # Fonction pour colorer la cellule si taille 5.0 = 1
        def color_size_5(val, taille):
            if taille == '5.0' and val == 1:
                return 'background-color: red'
            return 'background-color: white'

        # Appliquer le style
        styled_df = df_grouped.style.apply(
            lambda row: [color_size_5(row['Quantité totale'], row['Taille']) for _ in row],
            axis=1
        )

        # Afficher le tableau
        st.subheader(f"Stock pour la désignation : {designation}")
        st.dataframe(styled_df)
    else:
        st.write("Aucune donnée trouvée pour cette désignation.")
        ...
        if not df_family.empty:
            ...
            st.dataframe(df_family[
                             ['rayon', 'fournisseur', 'couleur', 'taille', 'designation', 'marque', 'ssfamille',
                              'Qté stock dispo', 'Valeur Stock']].style.apply(highlight_critical_stock, axis=None))
        ...
        with st.spinner("Chargement des données..."):
            ...
            if df is not None:
                ...
                with tab1:
                    ...
                    if not df_filtered.empty:
                        st.dataframe(df_filtered.style.apply(highlight_critical_stock, axis=None))
                    else:
                ...
                with tab3:
                    st.dataframe(filter_negative_stock(df.copy()).style.apply(highlight_critical_stock, axis=None))
                ...
                with tab5:
                    ...
                    for level, df_level in sidas_results.items():
                        ...
                        st.dataframe(df_level.style.apply(highlight_critical_stock, axis=None))

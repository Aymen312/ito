...
def display_specific_designations(df):
    ...
    suppliers = {
        ...
        # ... (autres fournisseurs conservés tels quels)
    }
    ...
    if 'selected_supplier' in st.session_state:
        ...
        if not df_filtered.empty:
            ...
            for designation in sorted(designations):
                ...
                # Gestion spéciale ASICS
                if supplier == "ASICS":
                    # Cas GEL-NIMBUS 27
                    if "GEL-NIMBUS 27" in designation.upper():
                        rayon = df_design['rayon'].iloc[0] if not df_design.empty else "HOMME"
                        
                        if rayon == "FEMME":
                            expected_sizes = [f"{x}.0" for x in range(5, 11)] + [f"{x}.5" for x in range(5, 10)]
                            display_name = "W GEL-NIMBUS 27"
                        else:
                            expected_sizes = [f"{x}.0" for x in range(7, 15)] + [f"{x}.5" for x in range(7, 13)]
                            display_name = "GEL-NIMBUS 27"
                    
                    # Cas GEL-TRABUCO 13 GTX W
                    elif "GEL-TRABUCO 13 GTX W" in designation.upper():
                        expected_sizes = [f"{x}.0" for x in range(5, 11)] + [f"{x}.5" for x in range(5, 10)]
                        display_name = "GEL-TRABUCO 13 GTX W"
                    
                    else:
                        # Logique standard pour autres modèles ASICS
                        is_woman = "W" in designation.upper()
                        expected_sizes = (
                            [f"{x}.0" for x in range(5, 11)] + [f"{x}.5" for x in range(5, 10)]
                            if is_woman else
                            [f"{x}.0" for x in range(7, 15)] + [f"{x}.5" for x in range(7, 13)]
                        )
                        display_name = designation
                
                # ... (logique pour autres fournisseurs)
                ...
                results.append({
                    'Modèle': display_name,
                    'Rayon': rayon if "GEL-NIMBUS" in designation else ("FEMME" if "W" in designation else "HOMME"),
                    'Tailles attendues': ", ".join(expected_sizes),
                    'Tailles manquantes': ", ".join(missing_sizes) if missing_sizes else "✓ Complet"
                })

            # Affichage du tableau
            results_df = pd.DataFrame(results)
            st.markdown(f"### {supplier} - Stock")
            st.dataframe(
                results_df.style
                .apply(lambda x: ['background: #E8F5E9' if x['Tailles manquantes'] == '✓ Complet' else '' for i in x], axis=1)
            ...
            col1, col2 = st.columns(2)
            with col1:
                if st.button("📊 Excel", key=f"excel_{supplier}"):
                    # ... (votre logique d'export Excel)
            with col2:
                if st.button("📄 CSV", key=f"csv_{supplier}"):

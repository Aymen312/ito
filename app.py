...
[01:19, 11/04/2025] ايمن: def display_designation_info(df, designation):
    ...
    def normalize_size(size):
        ...
        if '.' in size_str:
            ...
            size_str = f"{int_par…
[01:47, 11/04/2025] ايمن: def display_designation_info(df, designation):
    # Colonnes à afficher dans le tableau principal
    colonnes_a_afficher = ['barcode', 'taille', 'designation', 'Qté stock dispo']
    designation = designation.strip().upper()
    df['designation'] = df['designation'].fillna('')
    ...
    def normalize_size(size):
        ...
        if '.' in size_str:
            int_part, dec_part = size_str.split('.', 1)
    …
[18:15, 11/04/2025] ايمن: import streamlit as st
import pandas as pd
from io import BytesIO
